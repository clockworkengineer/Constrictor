...
from core.constants import CONFIG_TYPE, CONFIG_NAME, CONFIG_SOURCE, \
    CONFIG_DESTINATION, CONFIG_EXITONFAILURE, CONFIG_DELETESOURCE, CONFIG_RECURSIVE
...
def create_watcher_config() -> ConfigDict:
    ...
    config: ConfigDict = {CONFIG_NAME: "Copy File", CONFIG_TYPE: "CopyFile",
                          CONFIG_EXITONFAILURE:  False, CONFIG_DELETESOURCE: True,
...
def remove_source_destination(watcher_config: ConfigDict) -> None:
    ...
    if CONFIG_DESTINATION in watcher_config and \
        pathlib.Path(watcher_config[CONFIG_DESTINATION]).exists():
