...
class TestCoreWatcher:
    ...
    def test_watcher_with_a_valid_config_check_source_directory(
        self, watcher_config: ConfigDict
    ) -> None:
        assert pathlib.Path(watcher_config[CONFIG_SOURCE]).exists() == False
        watcher = Watcher(watcher_config)
        assert pathlib.Path(watcher_config[CONFIG_SOURCE]).exists() == True

    def test_watcher_with_a_valid_config_check_desination_directory(
        self, watcher_config: ConfigDict
    ) -> None:
        assert pathlib.Path(watcher_config[CONFIG_DESTINATION]).exists() == False
        watcher = Watcher(watcher_config)
        assert pathlib.Path(watcher_config[CONFIG_DESTINATION]).exists() == True
    ...
    def test_watcher_copy_a_single_file_from_source_to_destination(
        ...
        assert (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / f"test0.txt"
        ).exists() == False
    ...
    def test_watcher_copy_one_file_recursive_depth_one_deletesource(
        ...
        watcher = Watcher(watcher_config)
        watcher.start()
        source_path = pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "test.txt"
        create_test_file(source_path)
        self.__wait_for_processed_files(watcher, 1)
        watcher.stop()
        assert source_path.exists() == False
        assert (
            pathlib.Path(watcher_config[CONFIG_DESTINATION]) / "dir1" / "test.txt"
        ).exists() == True
    ...
    def test_watcher_copy_one_file_recursive_depth_two_deletesource(
        ...
        watcher.start()
        source_path = (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "dir2" / "test.txt"
        )
        create_test_file(source_path)
        self.__wait_for_processed_files(watcher, 1)
        watcher.stop()
        assert source_path.exists() == False
        assert (
            pathlib.Path(watcher_config[CONFIG_DESTINATION])
            / "dir1"
            / "dir2"
            / "test.txt"
        ).exists() == True
    ...
    def test_watcher_copy_two_files_recursive_depth_one_deletesource(
        ...
        watcher.start()
        source_path0 = (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "test00.txt"
        )
        create_test_file(source_path0)
        source_path1 = (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "test01.txt"
        )
        ...
        assert (
            pathlib.Path(watcher_config[CONFIG_DESTINATION]) / "dir1" / "test00.txt"
        ).exists() == True
        assert (
            pathlib.Path(watcher_config[CONFIG_DESTINATION]) / "dir1" / "test01.txt"
        ).exists() == True
    ...
    def test_watcher_deletesource_source_of_depth_one(
        ...
        self.__wait_for_processed_files(watcher, 1)
        watcher.stop()
        assert (pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1").exists() == False
        assert source_path.exists() == False
    ...
    def test_watcher_deletesource_source_of_depth_two(
        ...
        watcher.stop()
        assert (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "dir2"
        ).exists() == False
        assert (pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1").exists() == False
        assert (pathlib.Path(watcher_config[CONFIG_SOURCE])).exists() == True
        assert source_path.exists() == False
    ...
    def test_watcher_deletesource_two_files_source_of_depth_two(
        ...
        watcher.start()
        source_path0 = (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "dir2" / "test00.txt"
        )
        create_test_file(source_path0)
        source_path1 = (
            pathlib.Path(watcher_config[CONFIG_SOURCE]) / "dir1" / "test01.txt"
    ...
    def test_watcher_deletesource_one_file_root_check(
        ...
        watcher = Watcher(watcher_config)
        watcher.start()
        source_path = pathlib.Path(watcher_config[CONFIG_SOURCE]) / "test.txt"
        create_test_file(source_path)
        self.__wait_for_processed_files(watcher, 1)
        watcher.stop()
        assert (pathlib.Path(watcher_config[CONFIG_SOURCE])).exists() == True
    ...
    def test_watcher_copy_a_single_readonly_file_from_source_to_destination(
        ...
        assert source_path.exists() == False
        assert (
            pathlib.Path(watcher_config[CONFIG_DESTINATION]) / "test.txt"
        ).exists() == True
        assert watcher.files_processed == 1
